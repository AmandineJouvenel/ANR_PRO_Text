...
def ouvrir_csv(file: str) -> List[AnLine] : 
    ...
    with open (file, 'r', encoding='utf-8') as f:
        ...
        # S'adapter au format du fichier (csv ou tsv)
        ...
        # Initialiser une liste pour contenir les ligne annotées
        liste_lignes = []

        # Parcourir chaque ligne du fichier
        for row in reader:

            # Stocker les informations de chaque ligne dans une variable "line", qui est un objet AnLine
            line = AnLine(
                ID=row[0],
                charge=row[1],
                outil=row[2],
            ...
            # Ajouter la ligne à la liste des lignes annotées
